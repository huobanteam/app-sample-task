...
class ExceptionHandler
{
    ...
    public function getContent(FlattenException $exception)
    {
        ...
        if ($statusCode == 404) {
            $code = 500;
            $message = '伙伴任务应用暂时找不到您访问的页面';
        } else {
            $code = 500;
            $message = '伙伴任务应用暂时无法响应您的请求';
        }

        $warning_handler = new StreamHandler(storage_path('logs/system_error.log'), \Monolog\Logger::WARNING);
        $logger = new \Monolog\Logger('warning', array($warning_handler));
        $error = 'code:' . $statusCode . ';method:' . $_SERVER['REQUEST_METHOD'] . ';url:' . 'http://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
        $logger->error($error);
        ...
        $response = response()->json($result, $code);
        $response->send();
        return;
